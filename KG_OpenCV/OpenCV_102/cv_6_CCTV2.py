...
def get_diff_img(frame_a, frame_b, frame_c, threshold):
    ...
    k = cv2.getStructuringElement(cv2.MORPH_CROSS, (3,3)) # 3x3 교차형 필터 생성
    diff = cv2.morphologyEx(diff, cv2.MORPH_OPEN, k) # diff에 대해 MORPH_OPEN 방식으로 필터를 적용해 노이즈 제거하고 홈을 채움.
    # 영상에서 1인 부분의 갯수를 셈
...
while True:
    ...
    key = cv2.waitKey(30) #1s 동안 키입력 대기
    if key == ord('q'):
        break
...
#        ▼                                ▼
# ┌───────────────────┐            ┌───────────────────────┐
# │ record_counter =  │            │ (감지 없음)            │
# │ KEEP_RECORD_FRAMES│            │ 유지카운터 감소 체크    │
# └───────────────────┘            └───────────────────────┘
#        │                                │
#        ▼                                ▼
# ┌──────────────────────────────────────────────┐
# │ record_counter > 0 ? (녹화 유지 시간 남았나) │
# └──────────────────────────────────────────────┘
#        │Yes                             │No
#        ▼                                ▼
# ┌───────────────────────────────────────┐
