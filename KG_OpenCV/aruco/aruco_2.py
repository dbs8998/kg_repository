...
dertctor = aruco.ArucoDetector(aruco_dict, parameters)



# 비디오 캡처를 시작합니다.
cap = cv2.VideoCapture(0)
marker_length = 0.022  # 마커의 실제 크기 (미터 단위)
...
while True:
    ...
    # Aruco 마커를 탐지합니다.
    # (마커 코너 좌표, 마커의 ID, 탐지되지 않은 마커 후보군)
    # corners, ids, rejectedImgPoints = aruco.detectMarkers(gray, aruco_dict, parameters=parameters)
    corners, ids, rejectedImgPoints = dertctor.detectMarkers(gray)
    ...
    if ids is not None:
        ...
        for i in range(len(ids)):
            ...
            # 거리를 화면에 출력합니다.
            # distance[0]의 소수점2자리까지 값 미터, 텍스트 위치, 텍스트 크기 배욜 1, 초록, 텍스트 선 두께 2
